...
module.exports.blueprints = {
   ...
   * as long as a model exists with the same name as the controller, Sails
   * will respond with built-in CRUD logic in the form of a JSON API, including
   * support for sort, pagination, and filtering.
   */

  // Action blueprints speed up the backend development workflow by eliminating
  // the need to manually bind routes. When enabled, GET, POST, PUT, and DELETE
  // routes will be generated for every one of a controller's actions.
  //
  // If an `index` action exists, additional naked routes will be created
  // for it. Finally, all `actions` blueprints support an optional path
  // parameter, `id`, for convenience.
  //
  // For example, assume we have an EmailController with actions `send` and
  // `index`. With `actions` enabled, the following blueprint routes would be
  // bound at runtime:
  //
  ...
  // if you'd like to continue to use controller/action autorouting in a
  // production deployment, you must take great care not to inadvertently
  // expose unsafe/unintentional controller logic to GET requests.
  actions: true,
  ...
  // a conventional REST API on top of a controller's `find`, `create`,
  ...
  // Shortcut blueprints are simple helpers to provide access to a controller's
  // CRUD methods from your browser's URL bar.  When enabled, GET, POST, PUT,
  // and DELETE routes will be generated for the controller's`find`, `create`,
  // `update`, and `destroy` actions.
  //
  ...
  //  advantage of blueprint routing, even if you need to namespace your
  // API methods.
  //
  // * (NOTE: This only applies to blueprint autoroutes, not manual routes from
  //    `sails.config.routes`)
  //
  // For example, `prefix: '/api/v2'` would make the following REST blueprint
  // routes for a FooController:
  //
  // `GET /api/v2/foo/:id?`
  // `POST /api/v2/foo`
  // `PUT /api/v2/foo/:id`
